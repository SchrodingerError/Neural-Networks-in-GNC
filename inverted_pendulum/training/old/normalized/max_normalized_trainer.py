...
weight_functions = {
    'constant': {
        'function': lambda t: torch.ones_like(t) / torch.ones_like(t).max(),
        'description': 'Constant weight: All weights are 1, normalized by the max (remains 1)'
    },
    'linear': {
        'function': lambda t: (t / t.max()) / (t / t.max()).max(),
        'description': 'Linear weight: Weights increase linearly from 0 to 1, normalized by the max weight'
    },
    'quadratic': {
        'function': lambda t: ((t / t.max()) ** 2) / ((t / t.max()) ** 2).max(),
        'description': 'Quadratic weight: Weights increase quadratically from 0 to 1, normalized by the max weight'
    },
    'exponential': {
        'function': lambda t: (torch.exp(t / t.max() * 2)) / (torch.exp(t / t.max() * 2)).max(),
        'description': 'Exponential weight: Weights increase exponentially, normalized by the max weight'
    },
    'inverse': {
        'function': lambda t: (1 / (t / t.max() + 1)) / (1 / (t / t.max() + 1)).max(),
        'description': 'Inverse weight: Weights decrease inversely, normalized by the max weight'
    },
    'inverse_squared': {
        'function': lambda t: (1 / ((t / t.max() + 1) ** 2)) / (1 / ((t / t.max() + 1) ** 2)).max(),
        'description': 'Inverse squared weight: Weights decrease inversely squared, normalized by the max weight'
    }
}
